...
create or replace
trigger ejercicio6
before insert or update of cliente on cita
for each row
declare
  cuantas number;
  nombre cliente.nombre%type;
begin
  select count(*) into cuantas from reserva_habi
  where cliente = :new.cliente and fecha = :new.fecha;
  
  select nombre into nombre from cliente where nif = :new.cliente;
  
  if cuantas = 0 then
    raise_application_error(-20000, 'El cliente' ||nombre || ' no tiene reserva
    de habitacion en la feha' || :new.fecha);
  end if;
end;
